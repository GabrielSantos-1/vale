...
export function AdminSidebar() {
  ...
  <aside className="hidden w-72 shrink-0 border-r border-white/10 bg-sidebar text-white lg:flex lg:flex-col">
    {/* ✔ MANTER text-white → sidebar escura precisa contraste */}
    
    <div className="border-b border-white/10 px-6 py-5">
      <Link href="/admin/dashboard" className="flex items-center gap-3">
        
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15">
          {/* ✔ OK → efeito glass leve no fundo escuro */}
          <Wifi className="h-5 w-5 text-green-400" />
        </div>

        <div>
          <p className="text-lg font-bold tracking-tight text-white">
            {/* ✔ MANTER → título em fundo escuro */}
            Verde Vale
          </p>

          <p className="text-xs text-slate-300">
            {/* ⚠ pode melhorar */}
            {/* text-slate-300 → substituir por text-muted para padronizar */}
            {/* NOVO: text-muted */}
